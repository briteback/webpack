	...
	declarators.forEach(function(declarator) {
		switch(declarator.type) {
			case "VariableDeclarator":
				...
				} else if(declarator.id.type === "Identifier" && !_this.applyPluginsBailResult("var " + declarator.id.name, declarator)) {
					_this.scope.renames["$" + declarator.id.name] = undefined;
					_this.scope.definitions.push(declarator.id.name);
					if(declarator.init)
						_this.walkExpression(declarator.init);
				} else {
					_this.walkExpression(declarator.id);
					if(declarator.init)
						_this.walkExpression(declarator.init);
				}
				//} else {
